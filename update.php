...
function checkDefExtracted() {
  ...
  if (!$result) {
    ...
    $missing = $missing_t = $missing_e = $missing_x = [];
      ...
      if (!strcasecmp(substr($file, -4), '.def')) {
        ...
        if (!is_dir($full = "$extracted/$file")) {
          ...
          $mark = function ($hotkey, $ready)
              use ($file, $full, &$missing_t, &$missing_e, &$missing_x) {
            $ready ? writeFile("$full/ok-$hotkey", '') :
              (is_file("$full/ok-$hotkey") and unlink("$full/ok-$hotkey"));
            $ready or ${"missing_$hotkey"}[] = $file;
          };
    ...
    $result = compact('def', 'extracted', 'missing', 'missing_t', 'missing_e',
                      'missing_x');
  }
...
=> Extracting DEFs is currently semi-manual (and arguably troublesome).
   ...
   Part II. Extract Picture(s)
    ...
    1. Run DefPreview
    2. Uncheck Options > Extract In 24 Bits (if your DefPreview doesn't have this, you're good)
    3. Exit DefPreview

    4. If not already there, copy "Extract Pictures.ahk" from Workbench to the folder with DefPreview.exe
    5. Drag & drop the copied file onto AutoHotkeyU32.exe
    6. Wait until windows stop flashing
    7. Close DefPreview windows that remained opened
